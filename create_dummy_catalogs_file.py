...
def gen_richcl_output():
    npts_richness=5
    col_id = np.array(range(row_numbers))
    col_rich_vec= generate_vector(row_numbers*npts_richness, 'float', 1, 10).reshape(row_numbers,npts_richness)
    # or just: col_rich_vec = np.zeros((row_numbers,npts_richness), dtype='float')
    col_rich_err_vec = col_rich_vec.copy()
    col_rad_vec = col_rich_vec.copy()
    col_rich_vec_rs = col_rich_vec.copy()
    col_rich_err_vec_rs = col_rich_vec.copy()
    col_rad_vec_rs = col_rich_vec.copy()
    ...
    matrix = {'ID_CLUSTER': np.array(col_id, dtype=np.dtype('i8')),
               'RICHNESS_VEC': np.array(col_rich_vec,dtype=np.dtype('f4')),
               'RICHNESS_ERR_VEC': np.array(col_rich_err_vec, dtype=np.dtype('f4')),
               'RADIUS_VEC': np.array(col_rad_vec, dtype=np.dtype('f4')),
               'RICHNESS': np.array(col_rich, dtype=np.dtype('f4')),
               'RICHNESS_ERR': np.array(col_rich_err, dtype=np.dtype('f4')),
               'RADIUS': np.array(col_rad, dtype=np.dtype('f4')),
               'BKG_FRACTION': np.array(col_bkg_frac, dtype=np.dtype('f4')),
               'RICHNESS_VEC_RS': np.array(col_rich_vec_rs, dtype=np.dtype('f4')),
               'RICHNESS_ERR_VEC_RS': np.array(col_rich_err_vec_rs, dtype=np.dtype('f4')),
               'RADIUS_VEC_RS': np.array(col_rad_vec_rs, dtype=np.dtype('f4')),
               'RICHNESS_RS': np.array(col_rich_rs, dtype=np.dtype('f4')),
               'RICHNESS_ERR_RS': np.array(col_rich_err_rs, dtype=np.dtype('f4')),
               'RADIUS_RS': np.array(col_rad_rs, dtype=np.dtype('f4'))}
    return Table(matrix) 
...
def add_header(fitsfile, header):
    d=fitsio.read(fitsfile)
    clean_old_files(fitsfile)
    fitsio.write(fitsfile, d, header=header)
...
if __name__ == '__main__':
    ...
    member_amico_file = os.path.join(output_folder, 'member_amico_stub.fits')
    ...
    clean_old_files(member_amico_file)
    ...
    header_member_amico = gen_member_amico_header()
    ...
    data_member_amico = gen_member_amico_output()
    ...
    data_richcl.write(richcl_file)
    add_header(richcl_file, header_richcl)
    
    fitsio.write(member_amico_file, data_member_amico, header=header_member_amico)
